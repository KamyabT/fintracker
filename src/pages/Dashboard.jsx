...
const Dashboard = () => {
  ...
  const stats = [
    { title: "Total Income", amount: totalIncome, icon: <CircleDollarSign color="green" size={42} /> },
    { title: "Total Expense", amount: totalExpenses, icon: <CircleDollarSign color="red" size={42} /> },
    { title: "Net Balance", amount: netBalance, icon: <Wallet color="blue" size={42} /> },
    { title: "Saving Rate", amount: savingRate, icon: <PiggyBank color="purple" size={42} /> },
  ];
    ...
    <div className="flex flex-row bg-back-secondary">
      ...
      <main className="flex-1 flex-wrap px-5 py-5 overflow-y-auto">
        ...
        <section className="grid gap-4 md:grid-cols-4">
          {stats.map((stat) => <StatsBox stat={stat} key={stat.title} />)}
        </section>
